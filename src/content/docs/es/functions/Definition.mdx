...
- `nombre_de_la_funcion` : Nombre que se le da a la función para poder llamarla posteriormente.

- `argumentos_opcionales` : Parámetros que la función recibe como entrada. Pueden ser cero, uno o varios.

- `#Bloque de código a ejecutar` : El código que se ejecutará cuando se llame la función. `return`: Opcional. La instrucción `return` permite a la función devolver un valor a la parte del programa que la llamó. Si no se incluye `return`, la función devuelve None.
